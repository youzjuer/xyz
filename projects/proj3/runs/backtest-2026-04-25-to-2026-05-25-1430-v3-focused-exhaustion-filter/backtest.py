...
def score_at(code, name, rows, idx):
    ...
    # Focused exhaustion filter: avoid chasing near-20cm climax bars after an
    # already large short-term run. This specifically catches setups like
    # 688981 on 2026-05-25, which gapped up, closed near the high, and then
    # mean-reverted on T+1.
    if code.startswith(("300", "301", "688", "689")):
        if r1 >= 17.5 and r3 >= 0.14 and r5 >= 0.24 and intraday_strength >= 0.09 and range_pos >= 0.85:
            return None
    ...
    return {
        ...
        "t_close": row["close"],
        "t_range_pos": range_pos,
        "t_intraday_strength": intraday_strength,
    }
...
def write_outputs(payloads, trades):
    ...
    lines = [
        ...
        f"- Signal adjustment: focused exhaustion filter excludes 20cm names with near-limit T-day climax after large 3-day and 5-day runs.",
