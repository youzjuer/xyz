...
def fetch_daily_batch(codes):
    ...
    try:
        ...
        for item in data:
            ...
            if result:
                (CACHE_DIR / f"{code}.json").write_text(json.dumps(result, ensure_ascii=False), encoding="utf-8")
...
def backtest(payloads):
    ...
    for date in sorted(by_date):
        ...
        for payload, idx in by_date[date]:
            ...
            if s is None:
                continue
            # Daily-data fallback for 14:30: use T+1 open as entry proxy.
            # This avoids leaking T+1 close/high data into the decision.
            entry = nxt["open"]
            exit_px = nxt["close"]
            ...
            s["t1_date"] = nxt["date"]
            s["entry_proxy"] = entry
            s["exit_close"] = exit_px
            s["t1_high"] = nxt["high"]
            s["gross_return"] = exit_px / entry - 1
            s["high_return"] = nxt["high"] / entry - 1
            candidates.append(s)
        if not candidates:
            continue
...
def write_outputs(payloads, trades):
    ...
    lines = [
        ...
        f"- Data source: Sohu historical daily K-line API, fetched at runtime.",
        ...
        f"- Execution approximation: T-day signal, T+1 open entry proxy, T+1 close exit; cost: {COST_BPS} bps round trip.",
        f"- Important limitation: no 14:30 minute bars in this run, so this is a daily-bar approximation, not the final production backtest.",
        "",
        "## Summary",
        "",
        f"- Trades: {len(trades)}",
    ...
    for t in trades:
        lines.append(
            ...
            f"{t['score']:.2f} | {t['net_return']:.2%} | {t['high_return']:.2%} | {t['equity']:.4f} |"
        )
